...
BUILTIN_PATTERNS = [
    ...
    ("azure-sas-token", r"[?&]sig=[A-Za-z0-9%]{43,}(?:%3[Dd]|=)", "high",
    ...
    ("mailchimp-api-key", r"\b[0-9a-f]{32}-us\d{1,2}\b", "high",
    ...
    ("shopify-token", r"\bshp(?:at|ss|ca|pa)_[a-fA-F0-9]{32}\b", "high",
     "Shopify access token", "revoke the token"),
    ("postman-api-key", r"\bPMAK-[a-fA-F0-9]{24}-[a-fA-F0-9]{34}\b", "high",
    ...
    ("dropbox-token", r"\bsl\.[A-Za-z0-9_\-]{130,}\b", "high",
     "Dropbox access token", "revoke the token"),
    ("telegram-bot-token", r"\b\d{8,10}:AA[A-Za-z0-9_\-]{32,}\b", "high",
